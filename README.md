...
L'API respecte les contraintes de l'architecture **REST** (représentation par ressources au pluriel, verbes HTTP explicites). Toutes les opérations gravitent autour de la ressource principale `/users`.
...
Afin de livrer une application *Full-Stack*, FastAPI est configuré pour servir lui-même l'interface utilisateur.
...
La génération de l'entraînement ne délègue pas toute la logique métier au hasard du LLM. Elle utilise une communication stricte :
...
L'application est préparée contre les anomalies réseaux et les entrées utilisateur invalides :
...
* **Panne de l'IA (500 Internal Server Error) :** Si le LLM omet une clé obligatoire requise par le schéma Pydantic (ex: la clé `reps`), le bloc `try...except (json.JSONDecodeError, ValueError)` de `coach_service.py` intercepte l'exception, imprime la réponse brute dans les logs du serveur pour le débogage, et renvoie un message d'erreur clair à l'utilisateur.
* **Exceptions Réseau Ollama (502 / 503 / 504) :** Le service `ollama_service.py` capture les exceptions d'**HTTPX** (`TimeoutException`, `RequestError`, `HTTPStatusError`) pour renvoyer des statuts HTTP explicites (*"Ollama a mis trop de temps à répondre"*, *"Service indisponible"*) plutôt que de laisser l'API crasher de manière silencieuse.
* **Healthcheck Intelligent (Monitoring) :** La route `/health` effectue un ping asynchrone (avec un délai d'expiration de 2 secondes) directement vers le conteneur du LLM. Cela permet à l'infrastructure (et à Docker) de savoir si l'API est simplement allumée, ou si elle est **opérationnelle** et capable de générer des séances.
